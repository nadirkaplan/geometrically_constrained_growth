...
#time parameters 
T = int(round(25))  #  int(round(25))            # final time
dt=Constant(1e-8) #1e-6, 1e-7 are stable.
...
class Rate: #parameters of the growth rate
    def __init__(self):
        ...
        self.alpha2=Constant(0.5)
        self.alpha3=Constant(1.0)
        self.eta1=Constant(1.)
        self.eta21=Constant(1.)
        self.eta22=Constant(-1.)
        self.eta3=Constant(1.)
        self.eta41=Constant(3.)
        self.eta42=Constant(3.)
...
toutput[:]=[T/nframe*pp for pp in range(nframe)]
...
gg0='( sqrt(2.)*pi*sqrt(2.+pow(delta*kk, 2.)-pow(delta*kk, 2.)*cos(4*kk*pi*x[0])) )'

kn10temp='( (-4*delta*kk*kk*pi*cos(2*kk*pi*x[0]))/(-2-pow(delta*kk, 2.)+pow(delta*kk, 2.)*cos(4*kk*pi*x[0])) )'

kn10='('+kn10temp+'/('+gg0+'))'

kg0='( 2/(-2-pow(delta*kk, 2.)+pow(delta*kk, 2.)*cos(4*kk*pi*x[0])) )'

taug0='( (2*delta*kk*sin(2*kk*pi*x[0]))/(-2-pow(delta*kk, 2.)+pow(delta*kk, 2.)*cos(4*kk*pi*x[0])) )'
...
YY10='(-cos(2*pi*x[0]))'
YY20='(delta*cos(2*kk*pi*x[0]))'
YY30='sin(2*pi*x[0])'

tau10temp='2*pi*sin(2*pi*x[0])'
tau20temp='(-2*delta*kk*pi*sin(2*kk*pi*x[0]))'
tau30temp='2*pi*cos(2*pi*x[0])'

tau10=tau10temp+'/'+gg0
tau20=tau20temp+'/'+gg0
tau30=tau30temp+'/'+gg0

NN10temp='( -2.*delta*kk*pi*sin(2.*pi*x[0])*sin(2.*kk*pi*x[0]) )'
NN20temp='( -2.*pi )'
NN30temp='( -2*delta*kk*pi*cos(2*pi*x[0])*sin(2*kk*pi*x[0]) )'
...
u0=Expression((kg0, U0, kn20, gg0, kn10, taug0), 
              degree=1, delta=delta, kk=kk, alpha1=Rate().alpha1, alpha2=Rate().alpha2, 
...
q0=Expression((YY10, YY20, YY30, tau10, tau20, tau30, NN10, NN20, NN30), 
              degree=1, delta=delta, kk=kk, pi=pi)
...
F1=dt*inner(lambdau*grad(kg)/gg, grad(v1))*dx-dt*gg*U*v1*dx+dt*gg*v1*Utemp(kg, kn1, kn2, taug, param, H)*dx  

F2=((gg*kg+ccn*ggn*kgn+ccn_1*ggn_1*kgn_1+ccn_2*ggn_2*kgn_2+ccn_3*ggn_3*kgn_3+ccn_4*ggn_4*kgn_4)*v2*dx
    +dt*inner(ccr*Q2(U, gg), grad(v2))*dx-ccr*dt*f2(U, gg, kg, kn1, kn2, taug, kappaG)*v2*dx)
...
F3=((H(kn1, kn2)+ccn*H(kn1n, kn2n)+ccn_1*H(kn1n_1, kn2n_1)+ccn_2*H(kn1n_2, kn2n_2)+ccn_3*H(kn1n_3, kn2n_3)+ccn_4*H(kn1n_4, kn2n_4))*v3*dx
    -ccr*dt*f3(chi, U, kn1, kn2, H)*v3*dx)

F4=((gg+ccn*ggn+ccn_1*ggn_1+ccn_2*ggn_2+ccn_3*ggn_3+ccn_4*ggn_4)*w1*dx-ccr*dt*f4(U, gg, kg)*w1*dx)

F5=((gg*kn1+ccn*ggn*kn1n+ccn_1*ggn_1*kn1n_1+ccn_2*ggn_2*kn1n_2+ccn_3*ggn_3*kn1n_3+ccn_4*ggn_4*kn1n_4)*w2*dx
    -ccr*dt*f5(U, gg, kg, kn1, kn2, taug)*w2*dx)

F6=((gg*taug+ccn*ggn*taugn+ccn_1*ggn_1*taugn_1+ccn_2*ggn_2*taugn_2+ccn_3*ggn_3*taugn_3+ccn_4*ggn_4*taugn_4)*w3*dx
    -ccr*dt*f6(U, gg, kg, kn1, kn2, taug)*w3*dx)
...
filenameparam=shape+'_parameters_kk_'+str(kk)+'_'+d3+'.txt'
...
while  t<T: # and nn<100:
    ...
    if Nit<Nitmax:
        ...
        if t<=toutput[pp]<(t+float(dt)):
        
            print('index:', nn, '; iterations:', Nit, 'time:', float(t), 'time step:', float(dt))
        
            uarray[pp, :, :]= [ [u(ii)[jj] for ii in mesh.coordinates()] for jj in range(Neqs) ]
            qarray[pp, :, :]= [ [q(ii)[jj] for ii in mesh.coordinates()] for jj in range(NeqsAux) ]
...
X1array=qarray[0:pp, 0, :];  X2array=qarray[0:pp, 1, :]; X3array=qarray[0:pp, 2, :];
kappag_array=uarray[0:pp, 0, :]; U_array=uarray[0:pp, 1, :]; kN2_array=uarray[0:pp, 2, :];
gg_array=uarray[0:pp, 3, :]; kN1_array=uarray[0:pp, 4, :]; taug_array=uarray[0:pp, 5, :];

paramstring1='alpha1='+str(float(param.alpha1))+'; alpha2='+str(float(param.alpha2))+'; alpha3='+str(float(param.alpha3))+'\n'
paramstring2='eta1='+str(float(param.eta1))+'; eta21='+str(float(param.eta21))+'; eta22='+str(float(param.eta22))+'\n'
paramstring3='eta3='+str(float(param.eta3))+'; eta41='+str(float(param.eta41))+'; eta42='+str(float(param.eta42))+'\n'
paramstring4='lambda='+str(float(param.lambda1))+'\n'
paramstring5='kk='+str(kk)+'; delta='+str(delta)+'; chi='+str(float(chi))+'\n'
