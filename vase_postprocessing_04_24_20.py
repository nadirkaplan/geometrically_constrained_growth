...
shape='flower'
kk=4
d3 = '20_05_14'    #ntime max 87 for flower k=4, 04_14_20. beyond that U drops below zero.

theta=-2.*np.pi/float(kk)

rotmatrix=[np.matrix([[np.cos(theta*ii), -np.sin(theta*ii)], [np.sin(theta*ii), np.cos(theta*ii)]]) for ii in range(1, int(kk))]
...
aatemp=[[np.matmul(rotmatrix[ii], [X1array[:, jj], X2array[:, jj]]) for ii in range(int(kk)-1)] for jj in range(ntime)]
aatemp2=np.asarray(aatemp)

X1arrayF=np.zeros((nmesh*int(kk), ntime))
X2arrayF=np.zeros((nmesh*int(kk), ntime))
X3arrayF=np.zeros((nmesh*int(kk), ntime))
...
for ii in range(1, int(kk)):
    print(ii)
    X1arrayF[ii*nmesh:(ii+1)*nmesh, :]=aatemp2[:, ii-1, 0, :].transpose()
    X2arrayF[ii*nmesh:(ii+1)*nmesh, :]=aatemp2[:, ii-1, 1, :].transpose()
    X3arrayF[ii*nmesh:(ii+1)*nmesh, :]=X3array[0:nmesh, :]
...
# ntime=115 flower_kk_4_20_05_16
...
plt.ion()
ax= plt.axes(projection='3d')
[ax.plot3D(X1array[:, jj], X2array[:, jj], X3array[:, jj]) for jj in range(ntime)]
plt.show()
